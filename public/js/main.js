...
function actualizarPrecio(selectElement, index) {
    ...
    const precioInput = document.querySelector(
        `input[name="productos[${index}][precio_unitario]"]`
    );
...
function calcularTotal() {
    ...
    document.querySelectorAll("#productos_table tr").forEach((row, index) => {
        ...
        const precioUnitario =
            parseFloat(
                row.querySelector(
                    `input[name="productos[${index - 1}][precio_unitario]"]`
                )?.value
            ) || 0;
        const descuento =
            parseFloat(
                row.querySelector(
                    `input[name="productos[${index - 1}][descuento]"]`
                )?.value
            ) || 0;
    ...
    document.getElementById("cantidad_items").innerText =
        cantidadItems.toLocaleString("es-ES");
    document.getElementById("precio_neto").innerText =
        precioNeto.toLocaleString("es-ES", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
        });
    document.getElementById("iva").innerText = iva.toLocaleString("es-ES", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    });
    document.getElementById("descuento").innerText =
        totalDescuento.toLocaleString("es-ES", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
        });
    document.getElementById("precio_total").innerText =
    ...
    document.getElementById("input_descuento").value =
        totalDescuento.toFixed(2);
    document.getElementById("input_precio_total").value =
